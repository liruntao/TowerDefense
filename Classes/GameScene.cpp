...
bool GameScene::init()
{
	...
	closeItem->setPosition(Point(winSize.width - closeItem->getContentSize().width/2 ,
	...
	/*
	auto a = map->getLayer("goodsLayer")->getProperties();
	auto b = map->getProperties();
	log("1");*/
...
void GameScene::initUI()
{
 //layout继承自widget
    ...
    for (int i = 0; i < shop_scrollView->getChildren().size(); ++i)
    {
        ...
        buy_Sprite->addTouchEventListener(this,toucheventselector(GameScene::SpriteCallback,this));
   ...
   shop_btn->addTouchEventListener(this,toucheventselector(GameScene::menuShopCallback, this));
	

    comeOut = false;
}

void GameScene::SpriteCallback(cocos2d::Ref* pSender, TouchEventType type)
{
    Size winSize = Director::getInstance()->getWinSize();
	//widget是buy_sprite，就是那个图标，parent是buysprite所在的那个shop_layout，通过tag的值的运算
	//能计算出来所选择的。
	...
	case TouchEventType::TOUCH_EVENT_CANCELED:
            ...
            if( canBliud == true )
            {
                auto endPos =Point((widget->getTouchEndPos().x - bgOrigin.x)/bgSprite->getScale(), (widget->getTouchEndPos().y - bgOrigin.y)/bgSprite->getScale());
...
Point GameScene::convertTotileCoord(Point position)
{
...
Point GameScene::convertToScreenCoord(Point position)
{
    auto mapSize = map->getMapSize();
...
void GameScene::moveCheck(Point position, int tag)
{
    auto mapSize = map->getMapSize();
...
void GameScene::checkAndCreate(Point pos)
{
    map->getLayer("goodsLayer")->setTileGID(9, pos);
}

void GameScene::menuShopCallback(cocos2d::Ref* pSender, TouchEventType type)
{
    Size winSize = Director::getInstance()->getWinSize();
    switch (type)
...
void GameScene::onTouchesMoved(const std::vector<Touch*>& touches, Event  *event)
{
    ...
    if(touches.size() > 1)
    {
        Point point1 = touches[0]->getLocation();
