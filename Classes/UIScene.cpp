...
bool UIScene::init()
{
	...
	mainItem->setPosition(Point((size.width - startItem->getContentSize().width) / 2+50, size.height / 6));
	mainItem->setAnchorPoint(Point(0, 0));
	
	auto menu = Menu::create(startItem,mainItem, NULL);
...
void UIScene::menuStartCallback(Ref* pSender)
{
	...
	Director::getInstance()->replaceScene(CCTransitionFade::create(0.5, TollgateSelectScene::create()));
}
