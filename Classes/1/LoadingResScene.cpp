...
LoadingResScene::LoadingResScene() :numberOfLoadedRes(0), totalOfLoadedRes(22), progressBar(NULL)
{
}
...
ProgressTimer* LoadingResScene::addProgress(){
	...
	progressBar->addChild(progressBar);
	return progressBar;
}
...
void LoadingResScene::loadingCallBack(cocos2d::Texture2D* texture){
	numberOfLoadedRes++;
	log("numberOfLoaderRes=%d", numberOfLoadedRes);
}
