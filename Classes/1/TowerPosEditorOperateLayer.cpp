...
bool TowerPosEditorOperateLayer::init(TowerPosEditorLayer* layer) {
	...
	GoTOMainScene->addTouchEventListener(this, toucheventselector(TowerPosEditorOperateLayer::GotoMainScene));
...
void TowerPosEditorOperateLayer::GotoMainScene(Ref* target, TouchEventType type){
	if (type != TouchEventType::TOUCH_EVENT_ENDED){
		return;
	}
